// ech-workers.exe 内核代码 (最终完美融合版)
// 修复了 parseHTTPSRecord 函数中的 SvcParamKey，将其从 6 (新标准)
// 恢复为 5 (旧标准)，以匹配已知可以成功解析的“升级前”版本的行为。
// 这解决了“在DNS响应中未找到HTTPS记录”的根本问题。
...
// ... main 函数和 prepareECH 函数与上一版“融合版”完全相同 ...
func main() {
	flag.Parse()
...
// [核心终极修复] 将 parseHTTPSRecord 恢复为与“升级前”版本完全一致
func parseHTTPSRecord(data []byte) string {
	if len(data) < 2 {
		return ""
	}
	offset := 2
	// 跳过 TargetName 的逻辑 (与升级前版保持一致)
...
// ... 后续所有 WebSocket 和代理相关的代码，与之前的“融合版”完全相同，无需修改 ...
// (为保持完整性，全部列出)
// ======================== WebSocket 客户端 ========================

// core/core-json.go (v2.8 - Pure JSON)
...
func StartInstance(configContent []byte) (net.Listener, error) {
	...
	log.Printf("[Core] Titan Engine v2.8 (Pure JSON) Listening on %s", inbound.Listen)
	go func() {
		for {
...
func handleGeneralConnection(conn net.Conn, inboundTag string) {
	...
	wsConn, err := tryJSONHandshake(target, "proxy", firstFrame)
	if err != nil {
		log.Printf("[ERROR] JSON connection to %s failed: %v", target, err)
		return
	}
	
	log.Printf("[Success] Tunnel for %s established via JSON Protocol", target)
...
func tryJSONHandshake(target, tag string, frame []byte) (*websocket.Conn, error) {
	...
	connectMsg := fmt.Sprintf("X-LINK:%s|%s", target, base64.StdEncoding.EncodeToString(frame))
	jsonHandshake, _ := wrapAsJson([]byte(connectMsg))
	if err := wsConn.WriteMessage(websocket.TextMessage, jsonHandshake); err != nil { wsConn.Close(); return nil, err }
	
	_, msg, err := wsConn.ReadMessage()
	if err != nil { wsConn.Close(); return nil, err }

	ok, err := unwrapFromJson(msg)
	if err != nil || string(ok) != "X-LINK-OK" { wsConn.Close(); return nil, errors.New("json response invalid") }

	return wsConn, nil
}

// ... (pipeJSON, handleSOCKS5, handleHTTP, dialSpecificWebSocket 等函数保持不变) ...
